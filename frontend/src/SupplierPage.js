...
export default function SupplierPage() {
  ...
  // Handle edit mode - load supplier data when ID is in URL
  useEffect(() => {
    if (isEditMode && id && parties.length > 0) {
      const supplier = parties.find(p => p.partyid.toString() === id);
      if (supplier) {
        ...
        setFormData({
          ...
          AccountID: supplier.accountid || "25",
          GSTNum: supplier.gstnum || "",
        });
        setInitialFormData({
          PartyID: String(supplier.partyid ?? ""),
          PartyType: String(supplier.partytype ?? ""),
          PartyName: String(supplier.partyname ?? ""),
          ContactNo: String(supplier.contactno ?? ""),
          Address1: String(supplier.address1 ?? ""),
          Address2: String(supplier.address2 ?? ""),
          AccountID: String(supplier.accountid ?? "25"),
          GSTNum: String(supplier.gstnum ?? ""),
        });
      } else {
        // Supplier not found, redirect to list
        navigateToList();
      }
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error(err);
      const errorMessage = err.response?.data?.message || err.response?.data?.error || "Error deleting supplier";
      showToast(errorMessage, 'error');
    }
  };
  ...
  // Filter + pagination (Suppliers only: PartyType = 2)
  ...
  const goToPreviousRecord = () => {
    ...
    if (currentIndex > 0) {
      const previousSupplier = filteredParties[currentIndex - 1];
      navigateToEdit(previousSupplier.partyid);
    }
  };
  const goToNextRecord = () => {
    const currentIndex = getCurrentRecordIndex();
    if (currentIndex < filteredParties.length - 1) {
      const nextSupplier = filteredParties[currentIndex + 1];
      navigateToEdit(nextSupplier.partyid);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-2">
        ...
        <Breadcrumb 
          ...
          itemName={showForm ? (isNewMode ? "New Supplier" : `Edit Supplier: ${formData.PartyName}`) : null}
        />
        ...
        <div className="flex flex-col items-start mb-2">
          ...
          <h1 className="text-xl font-semibold">Master Suppliers</h1>
        </div>
