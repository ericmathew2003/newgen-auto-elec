...
import { FileText, PlusCircle, MinusCircle, CheckCircle, Save, RotateCcw, ArrowLeft } from 'lucide-react';
...
const initialMasterState = {
  creditNoteSerial: '',
  creditNoteDate: new Date().toISOString().substring(0, 10),
  sourceDocumentRef: '',
  narration: '',
};
...
const CreditNoteForm = () => {
  ...
  const [creditNoteDetails, setCreditNoteDetails] = useState([initialLineItem]);
  ...
  const fetchCreditNoteSerial = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/accounting/journals/credit-notes/serial/next`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMasterData(prev => ({ ...prev, creditNoteSerial: response.data.credit_note_serial }));
    } catch (error) {
      console.error('Error fetching credit note serial:', error);
      showMessage('Failed to generate credit note serial', 'error');
    }
  };
  ...
  const fetchCreditNoteData = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/accounting/journals/credit-notes/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setMasterData({
        creditNoteSerial: master.journal_serial || '',
        creditNoteDate: master.journal_date ? master.journal_date.substring(0, 10) : '',
      ...
      if (details.length > 0) {
        ...
        setCreditNoteDetails(detailsWithTempId);
      ...
      showMessage('Error loading credit note data', 'error');
      navigate('/accounts/credit-note');
    } finally {
  ...
  const { totalDebit, totalCredit, isBalanced } = useMemo(() => {
    const totalDebit = creditNoteDetails.reduce((sum, line) => sum + parseFloat(line.debitAmount || 0), 0);
    const totalCredit = creditNoteDetails.reduce((sum, line) => sum + parseFloat(line.creditAmount || 0), 0);
  ...
  const handleLineChange = (tempId, field, value) => {
    setCreditNoteDetails(prevDetails => prevDetails.map(line => {
  ...
  const addLine = () => {
    setCreditNoteDetails(prevDetails => [...prevDetails, { ...initialLineItem, tempId: nextTempId }]);
    setNextTempId(prevId => prevId + 1);
  };

  const removeLine = (tempId) => {
    if (creditNoteDetails.length === 1) {
      return showMessage("Credit note must have at least one line item.", 'error');
    }
    setCreditNoteDetails(prevDetails => prevDetails.filter(line => line.tempId !== tempId));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isBalanced) {
      showMessage(`Credit note is unbalanced. Debit (${totalDebit.toFixed(2)}) must equal Credit (${totalCredit.toFixed(2)}).`, 'error');
      return;
    }

    if (creditNoteDetails.some(line => !line.accountId || (line.debitAmount <= 0 && line.creditAmount <= 0))) {
    ...
    const payload = {
      credit_note_date: masterData.creditNoteDate,
      finyearid: parseInt(storedFYearId, 10),
      credit_note_serial: masterData.creditNoteSerial,
      source_document_ref: masterData.sourceDocumentRef,
      total_debit: totalDebit.toFixed(2),
      total_credit: totalCredit.toFixed(2),
      narration: masterData.narration,
      credit_note_details: creditNoteDetails.map(line => ({
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await axios.put(`${API_BASE_URL}/api/accounting/journals/credit-notes/${id}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        response = await axios.post(`${API_BASE_URL}/api/accounting/journals/credit-notes`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      ...
      const errorMessage = error.response?.data?.error || error.message || 'Failed to save credit note';
      ...
      <nav className="flex mb-6" aria-label="Breadcrumb">
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          <li className="inline-flex items-center">
            <button
              onClick={() => navigate('/accounts/credit-note')}
      ...
      <div className="w-full max-w-4xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-gray-500 mt-1">
            {isEditMode ? 'Update the credit note details below.' : 'Create a new credit note for refunds or adjustments.'}
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 mb-6">
            <h2 className="text-lg font-semibold mb-4 border-b pb-2 text-gray-700">Credit Note Header</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="creditNoteSerial" className="block text-sm font-medium text-gray-700 mb-1">
                  Credit Note Serial
                </label>
                <input
                  type="text"
                  id="creditNoteSerial"
                  value={masterData.creditNoteSerial}
                  readOnly
              ...
              <div>
                <label htmlFor="creditNoteDate" className="block text-sm font-medium text-gray-700 mb-1">
                  Credit Note Date
                </label>
                <input
                  type="date"
                  id="creditNoteDate"
                  required
                  value={masterData.creditNoteDate}
                  onChange={handleMasterChange}
                  disabled={(isEditMode && !canEdit('ACCOUNTS', 'CREDIT_NOTE')) || (!isEditMode && !canCreate('ACCOUNTS', 'CREDIT_NOTE'))}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 transition duration-150 shadow-sm"
                />
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  disabled={(isEditMode && !canEdit('ACCOUNTS', 'CREDIT_NOTE')) || (!isEditMode && !canCreate('ACCOUNTS', 'CREDIT_NOTE'))}
                  rows="2"
                  placeholder="Briefly describe the purpose of this credit note..."
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 resize-none transition duration-150 shadow-sm"
                />
              </div>
            </div>
          </div>
          ...
          {/* Credit Note Lines */}
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 mb-6">
            <h2 className="text-lg font-semibold mb-4 border-b pb-2 text-gray-700">Credit Note Lines</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {creditNoteDetails.map((line) => (
                    <tr key={line.tempId} className="hover:bg-green-50">
                      ...
                      <td className="p-2 whitespace-nowrap">
                        <input
                          ...
                          placeholder="0.00"
                          className="w-full p-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 text-sm text-right bg-red-50"
                        />
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <input
                          ...
                          placeholder="0.00"
                          className="w-full p-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 text-sm text-right bg-green-50"
                        />
                      </td>
                  ...
                  <tr>
                    <td colSpan="3" className="py-3 px-3 text-right font-bold text-gray-800">
                      <button 
                        type="button" 
                        onClick={addLine}
                        className="inline-flex items-center text-sm text-green-600 hover:text-green-800 transition font-semibold"
                      >
