...
export default function CustomerPage() {
  ...
  const startEditing = (party) => {
    const mappedData = {
      ...
      PartyType: party.partytype?.toString() || "1",
  ...
  const fetchParties = async () => {
    try {
      ...
      const customers = all.filter(p => String(p.partytype ?? '').toString() === '1');
  ...
  const handleSubmit = async (e, { addAnother = false } = {}) => {
    ...
    try {
      if (editingItem) {
        ...
        await axios.put(`http://localhost:5000/api/party/edit/${editingItem.PartyID}`, rest);
        showNotice('success', 'Customer updated successfully');
      } else {
        await axios.post("http://localhost:5000/api/party/add", formData);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6">
        ...
        {!showForm ? (
          ...
          <FormWrapper
            title={editingItem ? "Customer / Edit" : "Customer / New"}
            onClose={closeForm}
