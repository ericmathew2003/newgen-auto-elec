...
const AccJournalEntryForm = () => {
  ...
  const fetchJournalSerial = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/accounting/journals/serial/next`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setMasterData(prev => ({ ...prev, journalSerial: response.data.journal_serial }));
    } catch (error) {
  ...
  const fetchJournalData = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/api/accounting/journals/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleLineChange = (tempId, field, value) => {
    setJournalDetails(prevDetails => prevDetails.map(line => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      journal_serial: masterData.journalSerial,
    ...
    try {
      ...
      let response;
      if (isEditMode) {
        response = await axios.put(`${API_BASE_URL}/api/accounting/journals/${id}`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        response = await axios.post(`${API_BASE_URL}/api/accounting/journals`, payload, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      ...
      const errorMessage = error.response?.data?.error || error.message || 'Failed to save journal entry';
      ...
      <div className="w-full max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 mb-6">
            <h2 className="text-lg font-semibold mb-4 border-b pb-2 text-gray-700">Journal Header</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label htmlFor="journalSerial" className="block text-sm font-medium text-gray-700 mb-1">
                  Journal Serial
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="journalDate" className="block text-sm font-medium text-gray-700 mb-1">
                  Journal Date
                </label>
                <input
                  type="date"
                  ...
                  disabled={(isEditMode && !canEdit('ACCOUNTS', 'JOURNAL_VOUCHER')) || (!isEditMode && !canCreate('ACCOUNTS', 'JOURNAL_VOUCHER'))}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500 transition duration-150 shadow-sm"
                />
              </div>
              ...
              <div className="md:col-span-3">
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500 resize-none transition duration-150 shadow-sm"
                />
              </div>
            </div>
          </div>
          ...
          {/* Journal Lines */}
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 mb-6">
            <h2 className="text-lg font-semibold mb-4 border-b pb-2 text-gray-700">Journal Lines</h2>
                    ...
                    <tr key={line.tempId} className="hover:bg-red-50">
                      ...
                      <td className="p-2 whitespace-nowrap">
                        <input
                          ...
                          placeholder="0.00"
                          className="w-full p-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500 text-sm text-right bg-red-50"
                        />
                      </td>
                      <td className="p-2 whitespace-nowrap">
                        <input
                          ...
                          placeholder="0.00"
                          className="w-full p-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500 text-sm text-right bg-green-50"
                        />
                      </td>
                  ...
                  <tr>
                    <td colSpan="3" className="py-3 px-3 text-right font-bold text-gray-800">
                      <button 
                        type="button" 
                        onClick={addLine}
                        className="inline-flex items-center text-sm text-red-600 hover:text-red-800 transition font-semibold"
                      >
                  ...
                  <tr className={isBalanced ? 'bg-green-50' : 'bg-red-50'}>
                    <td colSpan="6" className="py-3 px-3 text-center">
                      <div className={`font-bold text-sm flex items-center justify-center ${
                        ...
                        {isBalanced 
                          ? 'Journal is Balanced' 
                          : `Unbalanced by: ${Math.abs(totalDiff).toFixed(2)} ${totalDiff > 0 ? '(Credit needed)' : '(Debit needed)'}`
                        }
                      </div>
                    </td>
                  </tr>
              ...
              >
                ...
                <span>
                  {isLoading 
                    ...
                    : (isEditMode ? 'Update Journal' : 'Post Journal Entry')
                  }
                </span>
              </button>
            )}
