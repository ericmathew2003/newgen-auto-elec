...
export default function SalesPage() {
  ...
  // Status management - 3-step workflow
  const getSalesStatus = () => {
    if (!editing?.tranid) return 'DRAFT';
    if (header.Is_Posted) return 'POSTED';
    if (header.Is_Confirmed) return 'CONFIRMED';
    return 'DRAFT';
  };

  const salesStatus = getSalesStatus();
  const isPosted = salesStatus === 'POSTED';
  ...
  const handleConfirm = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/sales/${editing.tranid}/confirm`, {}, getAuthHeaders());
      setHeader(h => ({ ...h, Is_Confirmed: true }));
      await fetchSales();
      setNotice({ open: true, type: 'success', message: 'Invoice confirmed successfully' });
    } catch (e) {
      console.error(e);
      setNotice({ open: true, type: 'error', message: e.response?.data?.error || 'Failed to confirm invoice' });
    }
  };
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {currentRecords.map((r) => (
                  ...
                  >
                    ...
                    <td className="p-2 text-center">
                      <input type="checkbox" checked={!!r.is_confirmed} readOnly onClick={(e) => e.stopPropagation()} />
                    </td>
                    <td className="p-2 text-center">
                ...
                {currentRecords.length === 0 && (
                  <tr><td colSpan={11} className="p-4 text-center text-gray-500">No records</td></tr>
                )}
              </tbody>
            </table>
        ...
        <div className="space-y-4 max-w-[104rem] mx-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* Confirm Button - Show when in Draft mode */}
                {editing?.tranid && isDraft && canEdit('INVENTORY', 'SALES') && (
                  <button
                    disabled={saving}
                ...
                {/* Post Button - Show when Confirmed */}
                {editing?.tranid && isConfirmed && canEdit('INVENTORY', 'SALES') && (
                  <button
                    disabled={saving}
                    onClick={handlePost}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold shadow ${isDraft ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-500'
                  }`}>
                  DRAFT
                </span>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold shadow ${isConfirmed ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-500'
                  }`}>
                  CONFIRMED
                </span>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold shadow ${isPosted ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-500'
                  }`}>
                  POSTED
                </span>
              </div>
          ...
          <div className="border rounded">
            <div className="flex items-center justify-between p-2">
              ...
              {((editing?.tranid && canEdit('INVENTORY', 'SALES')) || (!editing?.tranid && canCreate('INVENTORY', 'SALES'))) && (
                <button onClick={addRow} className="px-3 py-1 bg-blue-600 text-white rounded" disabled={isLocked}>Add Row</button>
              )}
            </div>
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                  ...
                  return (
                    <tr key={idx} className="border-t">
                      ...
                      <td className="p-2 text-right">
                        <input type="number" value={it.Qty} onChange={e => updateItem(idx, 'Qty', e.target.value)} className="border p-1 rounded w-32 text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" disabled={isLocked} />
                      </td>
                      <td className="p-2 text-right">
                        <input type="number" value={it.Rate} onChange={e => updateItem(idx, 'Rate', e.target.value)} className="border p-1 rounded w-32 text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" disabled={isLocked} />
                      </td>
                      ...
                      <td className="p-2 text-right">
                        {canDelete('INVENTORY', 'SALES') && (
                          <button onClick={() => removeRow(idx)} className="px-2 py-1 border rounded" disabled={isLocked}>Delete</button>
                        )}
                      </td>
                    </tr>
                  );
              ...
              <div className="flex justify-end">
                <div className="w-full md:w-auto md:min-w-[320px] space-y-2 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    }} className="border p-1 rounded w-28 text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" disabled={isLocked} />
                  </div>
